...
public class Coding implements IExecutor {
    ...
    public RC setConfig(String configFileName){
        ...
        try{
            Syntaksis synt = new Syntaksis(new BufferedReader(new FileReader(configFileName)), logger, grammar.delimiter());
            Coding_Semantic seman = new Coding_Semantic(synt.Result());
            mode = seman.Mode();
            if(mode == CodingGrammar.MODE.ENCODING){
